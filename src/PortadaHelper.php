...
class PortadaHelper {
    public static function obtenerUrl($urlGuardada, $titulo, $idExterno = '') {
        ...
        // 1. Si hay ID de Google, probamos la URL directa de la API
        ...
        // 2. Si hay una URL de OpenLibrary o externa limpia
        if (!empty($urlGuardada) && str_starts_with($urlGuardada, 'http')) {
            return str_replace('http://', 'https://', $urlGuardada);
        }

        // 3. Si no hay URL válida, devolvemos una portada generada al vuelo con el título
